...
public class JwtTenantUserValidationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        log.info(">> JwtTenantUserValidationFilter executed for: " + request.getRequestURI() + "with operation " + request.getMethod());

        String authHeader = request.getHeader("Authorization");

        if (request.getRequestURI().startsWith("/api/tenants/swagger-ui") ||
                request.getRequestURI().startsWith("/api/tenants/v3/api-docs")) {
        ...
        try {
            ...
            if (request.getRequestURI().contains("/events") && "POST".equalsIgnoreCase(request.getMethod())) {
                log.debug("Skipping JWT/tenant/user validation for POST /events");
                filterChain.doFilter(request, response);
                return;
            }
            if(request.getRequestURI().contains("/policy") && "GET".equalsIgnoreCase(request.getMethod())) {
                log.debug("Skipping JWT/tenant/user validation for GET /policy");
                filterChain.doFilter(request, response);
                return;
            }
            if(request.getRequestURI().contains("/extension-policy") && "GET".equalsIgnoreCase(request.getMethod())) {
                log.debug("Skipping JWT/tenant/user validation for GET /policy");
                filterChain.doFilter(request, response);
                return;
            }
            ...
            // Use eager-loading query to fetch user with all associations (groups, roles, scopes)
            // in a single DB round-trip to avoid LazyInitializationException outside session
            Optional<User> userOpt = userRepository.findByEmailAndTenantIdWithScopes(userFromRequest.getEmail(), tenantId);
            if (userOpt.isEmpty()) {
            ...
            // Extract Keycloak session ID (sid) and user ID (sub) from JWT.
            // For SSO users (Azure AD), keycloakUserId may not be persisted in DB yet —
            // fall back to the JWT 'sub' claim, which is always the Keycloak user ID.
            try {
                com.nimbusds.jwt.JWTClaimsSet claims = jwtUtil.decodeToken(authHeader.substring(7));
                if (claims != null) {
                    String sid = claims.getStringClaim("sid");
                    loggedInUser.setSessionId(sid);
                    log.debug("Extracted session ID '{}' from JWT for user '{}'", sid, email);

                    if (loggedInUser.getKeycloakUserId() == null) {
                        String sub = claims.getSubject();
                        loggedInUser.setKeycloakUserId(sub);
                        log.debug("keycloakUserId not in DB for SSO user '{}', using JWT sub '{}'", email, sub);

                        // Backfill the DB so future requests don't need the JWT fallback
                        if (sub != null) {
                            try {
                                user.setKeycloakUserId(sub);
                                userRepository.save(user);
                                log.info("Persisted keycloakUserId='{}' for SSO user '{}'", sub, email);
                            } catch (Exception saveEx) {
                                log.warn("Failed to persist keycloakUserId for user '{}': {}", email, saveEx.getMessage());
                            }
                        }
                    }
                }
            } catch (Exception e) {
                log.warn("Failed to extract JWT claims for user '{}': {}", email, e.getMessage());
            }
            ...
            // Set CurrentUserHolder for audit logging
            CurrentUserHolder.setContext(tenantId, user.getPkUserId(), user.getUserName());
            ...
            // Clear CurrentUserHolder to prevent thread-local leakage
...
//        log.info("Feature flag cache refresh complete: managedPaths={} tenantEntries={}", this.managedApiPaths.size(), this.tenantPathEnabledCache.size());
    ...
    private Set<String> resolveUserScopes(User user) {
        ...
        // Normalize tenant type once
        String userTenantType = user.getTenant()
                .getTenantType()
                .trim()
                .toUpperCase();
        ...
        Set<String> scopes = user.getMappedGroups()
                ...
                .flatMap(role ->
                        Optional.ofNullable(role.getScopes())
                                .orElse(Collections.emptySet())
                                .stream()
                )
        ...
        log.debug(
                "resolveUserScopes for user='{}' tenantType='{}' resolved {} scopes",
                user.getEmail(),
                userTenantType,
                scopes.size()
        );
